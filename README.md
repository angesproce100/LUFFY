...
│   ├── deepscaler/        # Scaling utilities (⚠️ Gemini integration pending)
...
- OpenAI API calls are implemented; logging, batch prompts, and timeout configuration remain TODOs
...
- **API Integration**: Implement Gemini support; add OpenAI logging, batch prompts, and timeout configuration
...
The existing README TODO list, updated only for Python changes in `main..dev`: **215 TODO items across 63 files**. Relative to the `main` baseline, 11 completed TODOs were removed, 18 line numbers were updated, and no new TODOs were introduced.

Each checkbox records a TODO marker, not necessarily an unimplemented feature. Pre-existing TODOs omitted from the baseline are not added by this scoped update; surrounding source provides context for multiline notes.
